---
name: shipjaw
description: Creates a named Shipjaw project folder, switches the working directory into it, and explains which skill to run next (prompt → build → ask; adopt; upgrade; challenge). Slash-only onboarding. Use when the user runs /shipjaw, start a Shipjaw project, nouveau projet Shipjaw, créer un dossier projet, need a clean project home before coding, or bootstrap workspace folder setup. Do not scaffold Next, write source-prompt.md, or create knowledge-base/ here. If documentation/knowledge-base/ already exists → shipjaw-ask (or shipjaw-upgrade / shipjaw-challenge as needed); existing app without KB → shipjaw-adopt; idea polishing → shipjaw-prompt; ready prompt → shipjaw-build.
...
Do **not** start prompt-craft, build, adopt, upgrade, or challenge unless
the user explicitly asks in the same message after onboarding. Hand off
cleanly.
