---
name: skill-my-app
description: Scaffold a brand-new TypeScript/Next.js site/app (NestJS only when needed) from a product prompt — docs-first, tests, security, clean architecture. Trigger for initial build only. Do not use for small CSS tweaks, non-TS repos, or when documentation/knowledge-base/ already exists (use ask-my-app instead).
---
...
`ask-my-app` — never re-run this skill on an existing KB.

**Operating principles:** `references/skill-principles.md` (1–16).
...
- `documentation/knowledge-base/` already exists → `ask-my-app`
...
4. **Tests** — Vitest for domain/application; Playwright + axe +
   keyboard/focus smoke for critical flows.
...
6. **Documentation** — committed `documentation/`. No app code before
   `INDEX.md` + phase-01. Stamp `scaffolded-with: skill-my-app@VERSION`.
...
If the repo looks like an older skill-my-app project, read
...
- `templates/` · `templates/scaffold/` · `VERSION`
